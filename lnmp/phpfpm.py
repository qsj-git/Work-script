...
def phpfpm():
    ...
    # 安装php-7.4.30
    if os.path.isfile("php-7.4.30.tar.gz"):
        subprocess.run("tar -zxf php-7.4.30.tar.gz", shell=True)
        # 编译
        instat = subprocess.run('''cd ./php-7.4.30 && ./configure --prefix=/usr/local/php-fpm7.4 --with-config-file-path=/usr/local/php-fpm7.4/etc --with-mysqli=mysqlnd --enable-pdo --with-pdo-mysql=mysqlnd --with-iconv-dir=/usr/local/ --enable-fpm --with-fpm-user=www --with-fpm-group=www --with-pcre-regex --with-zlib --with-bz2 --enable-calendar --with-curl --enable-dba --with-libxml-dir --enable-ftp --with-gd --with-jpeg-dir --with-png-dir --with-zlib-dir --with-freetype-dir --enable-gd-jis-conv --with-mhash --enable-mbstring --disable-opcache --enable-pcntl --enable-xml --disable-rpath --enable-shmop --enable-sockets --enable-zip --enable-bcmath --with-snmp --disable-ipv6 --with-gettext --disable-rpath --disable-debug --enable-embedded-mysqli --with-mysql-sock=/usr/local/mysql >> ../../log/php.log''',
                                shell=True)

        if "0" == str(instat)[-2]:
            # 安装
            subprocess.run("cd php-7.4.30 &&  make && make install >> ../../log/php.log", shell=True)
            # 拷贝ini 等文件
            os.popen("cp php-7.4.30/php.ini-production /usr/local/php-fpm7.4/etc/php.ini")
            os.popen("cp php-7.4.30/sapi/fpm/init.d.php-fpm /etc/init.d/php-fpm && chmod +x /etc/init.d/php-fpm")
            os.popen("cp /usr/local/php-fpm7.4/etc/php-fpm.conf.default /usr/local/php-fpm7.4/etc/php-fpm.conf")
            time.sleep(2)

            # 修改php-fpm.conf配置文件
            # newfile1 = []
            # with open("/usr/local/php-fpm7.4/etc/php-fpm.conf", 'r', encoding="utf-8") as file1:
            # phpid = "pid = run/php-fpm.pid"
            # phpinclude = "include=/usr/local/php-fpm7.4/etc/php-fpm.d/*.conf"
            ...
            #         else:
            #             newfile1.append(i)
            # with open("/usr/local/php-fpm7.4/etc/php-fpm.conf", 'w', encoding="utf-8") as file2:
